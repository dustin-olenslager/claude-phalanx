...
if [ -n "$latest" ] && [ "$latest" != "$cur" ]; then
  # SECURITY (audit 2026-08-17): the tag about to be checked out + executed (install.sh) is
  # REMOTE CODE. Anyone able to push a v* tag to origin would get code run under the operator's
  # shell at the next session -- origin-push == RCE. Require a VALID signature on the tag before
  # trusting it. Installs not yet signing releases can opt back into the old download-then-run
  # behavior with: export PHALANX_ALLOW_UNSIGNED_TAGS=1
  if [ "${PHALANX_ALLOW_UNSIGNED_TAGS:-0}" != "1" ] && ! git tag -v "$latest" >/dev/null 2>&1; then
    exit 0  # unsigned or bad signature -> refuse to auto-execute (fail closed, silent)
  fi
