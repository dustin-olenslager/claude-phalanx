...
- **`storageState`** starts every journey already signed in — the only way to preview an
  ...
  the recorder prints an unmissable warning naming the resolved path and records
  signed-out instead of failing the pass.
...
`docs/claude/in-progress.d/` fragments (Panoply / ADR-0006), each fragment is classified by
