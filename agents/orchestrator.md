...
## Done means done
Unit done when its check passes (build green / test pass / criteria met) — not when code was written. Keep dispatching workers (fix→verify→fix) until green or a real blocker needs the human. Never declare done on unverified work. On a real blocker (missing creds, ambiguous spec, dead external dep), write `BLOCKED: <reason>` to PROGRESS.md and stop — the outer loop halts for the human. **VERIFY BEFORE YOU BLOCK — a blocker you CAN check, you MUST check; never assume.** Before writing a provisioning / access / missing-resource BLOCKED (secrets, runner, creds, env, "not configured"), run the command that PROVES it and CITE that evidence in the BLOCKED line: secrets → `gh secret list`; a CI failure → the ACTUAL error from `gh run view <run> --log` (or `gh run rerun --failed` if logs expired), never a guess at the cause; a runner/service → its status; a file/cred → `ls`/read its source. An unverified "not provisioned"/"not a code bug"-style BLOCKED is a DEFECT, not a valid halt — it is the #1 lazy exit and it wastes the operator's time (real case: a `BLOCKED: secrets not provisioned` when `gh secret list` showed all four set months earlier). Pull the real error first; block only on what you confirmed, and name the SPECIFIC thing the operator must do.
...
1. **Merge — only if the repo opted in.** Look for `.phalanx-automerge` at the repo root.
   ...
   - **After a merge lands, delete the branch — both sides.** `git branch -d task/<slug>` and, if it was pushed, `git push origin --delete task/<slug>`. A merged branch left behind is the stale-branch pile the operator keeps finding (200+ in one repo). The supervisor's end-of-run GC (`phalanx-gc.sh`) also sweeps merged/squash-merged branches, but do not rely on it — clean up what you merged.
   - **Retire the queue fragment in the same change.** If the repo keeps `docs/claude/in-progress.d/<slug>.md`, the merge (or the PR that will merge) must delete that fragment and add its row to `docs/claude/completed-features.md` — a fragment that outlives its merge is a lie the next session plans against (38 of them piled up in one repo). `phalanx-docs-reconcile.sh --apply` does exactly this and is the shape to follow; on the PR path, include it in the PR.
   - **Leave the primary tree on `main`.** Never end a pass with the primary checkout parked on `task/<slug>`; the supervisor returns a CLEAN primary to `main` after every pass, but a dirty one it cannot touch — so commit or stash before you stop.
...
The supervisor runs each pass in its own git worktree under `.claude/worktrees/wt-<pass>-<stamp>/` — created by `run-work.sh` DETACHED at `main` with deps installed from the lockfile (or the repo's `.phalanx-worktree-setup`), and removed by the supervisor on every exit path. Your cwd IS that worktree; `PHALANX_REPO` is the primary. Create `task/<slug>` from the detached HEAD (`git checkout -b task/<slug>`) — never commit onto a detached HEAD or a `worktree-*` scratch branch (that work is invisible to the loop and gets swept). Three rules:
- **Restore salvaged WIP first.** If `$MAIN/PROGRESS.md` has a `WIP-STASH:` line, a prior pass ended with uncommitted work and the supervisor stashed it (`git stash list` shows the `phalanx-wip:` entry — worktrees share the stash). BEFORE resuming the task: in YOUR worktree check out the branch named on that line, `git stash pop` the matching entry, then delete the WIP-STASH line from `$MAIN/PROGRESS.md`. That stash IS the previous pass's real code — restoring it beats rebuilding from the prose checkpoint.
