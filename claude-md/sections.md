...
- INTERACTIVE SHIP CARVE-OUT (governs the rest of §17): when the OPERATOR is present and explicitly invokes `/ship` — or directly says push / merge-to-main / deploy — the operator IS the review. Proceed with merge-to-main + push + deploy in-session: do NOT deflect to a PR, do NOT require a `.phalanx-automerge` marker, do NOT halt for confirmation. The opt-in markers, the "open a PR" rule, and "BLOCKED = halt" below govern the UNATTENDED supervisor loop (`PHALANX_SUPERVISOR`/`PHALANX_ONESHOT`), NOT an operator-driven interactive ship — the loop-integrity gate enforces the same split mechanically (rules 5c/5e fire only for the unattended loop). Operator-risk HALT still applies for genuine data-loss / irreversible-prod, and a DB-migration merge WARNS (never blocks) interactively.
...
- ONE BACKLOG (ADR-0004, Accepted 2026-08-30 — DECIDED, sweep not yet landed): the single backlog is `docs/claude/in-progress.d/<slug>.md`, one committed fragment per task, carrying `status:` / `req:` / `risk:` frontmatter and a `Next step:` line. Panoply reads and writes the same files, so the loop's queue stops being invisible to review, to GitHub, and to every non-Claude agent. **Until the sweep lands, `TASKS.md` remains the operative mechanism described below** — do not hand-migrate a repo ahead of the scripts, or the loop will not see its own backlog. After the sweep, `TASKS.md` is read-only fallback for an unmigrated repo and is never written again.
...
- Checkpoint = `<project>/PROGRESS.md`. `BLOCKED: <reason>` halts the loop for the human. VERIFY BEFORE YOU BLOCK: a blocker you can check, you MUST check — before a provisioning/access/missing-resource BLOCKED (secrets/runner/creds/CI failure) run the command that proves it (`gh secret list`, `gh run view --log`, runner status, `ls`) and CITE the evidence + the SPECIFIC operator action. An unverified "not provisioned"/"not a code bug" guess is a DEFECT, not a valid halt.
