...
7. **The `.claude/rules/` module governing what you are about to touch** — `testing.md`,
   `error-handling.md`, `ai-features.md`, etc. Plain markdown; open the one that applies.
...
work**, and **verify (test + typecheck + lint) before you commit.**
...
- This is enforced **mechanically** where the project has wired it: this repo has **no import-boundary
  linter yet** — the layer map in `.claude/rules/clean-architecture.md` is adopt-as-target. Wiring one
  (dependency-cruiser / import-linter / ArchUnit per stack) is a recommended manual setup step; until
  it exists, dependency direction is enforced by review. See "Enforcement — the honest version" below.
